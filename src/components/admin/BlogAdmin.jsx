...
import {
    HiPlus, HiPencil, HiTrash, HiX, HiSave, HiUpload,
    HiEye, HiDocumentText
} from 'react-icons/hi'
...
const MarkdownEditor = ({ value, onChange, placeholder }) => {
    ...
    const renderPreview = (text) => {
        ...
        return text
            ...
            .replace(/`(.*?)`/g, '<code class="bg-dark-700 px-1 py-0.5 rounded text-accent-primary text-sm">$1</code>')
            .replace(/^- (.*$)/gm, '<li class="text-white/80 ml-4">$1</li>')
            .replace(/\n/g, '<br/>')
    }
...
const BlogAdmin = ({ posts, onSave, onDelete, onClose }) => {
    ...
    const categories = ['Development', 'Design', 'Animation', 'CSS', 'React', 'Tutorial', 'JavaScript', 'TypeScript']
                ...
                >
                    ...
                    <div className='px-5 md:px-6 py-5 md:py-6 space-y-5 overflow-y-auto flex-1 pb-24 bg-dark-900/20'>
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                            ...
                            <div>
                                <label className='block text-white/80 text-sm font-medium mb-2'>Read Time</label>
                                <input
                                    type='text'
                                    name='readTime'
                        ...
                        <div>
                            ...
                            />
                            <p className='text-white/40 text-xs mt-1'>{formData.excerpt?.length || 0}/200 characters</p>
                        </div>
                        ...
                        <div>
                            <label className='block text-white/80 text-sm font-medium mb-2'>
                                ...
                                <span className='text-white/40 text-xs ml-2'>(Markdown supported)</span>
                            </label>
                            ...
                            <div className='flex border-b border-white/10 mb-0'>
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm font-medium transition-all ${activeTab === 'content'
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm font-medium transition-all ${activeTab === 'preview'
                                        ? 'text-accent-primary border-b-2 border-accent-primary'
                                        : 'text-white/50 hover:text-white'}`}
                                >
                                    Preview
                            ...
                            {activeTab === 'content' ? (
                                ...
                                <div className='w-full min-h-[300px] px-4 py-3 bg-dark-800 border border-white/10 rounded-lg overflow-auto prose prose-invert max-w-none'>
                                    {formData.content ? (
                                        ...
                                        <p className='text-white/40 italic'>No content yet. Switch to Write tab to add content.</p>
                                    )}
                                </div>
                            )}
                        </div>
                        ...
                        <div className='flex items-center gap-4 text-sm text-white/50'>
                            <span>{formData.content?.split(/\s+/).filter(w => w.length > 0).length || 0} words</span>
                            <span>•</span>
    ...
    const renderMarkdown = (text) => {
        if (!text) return ''
        return text
            .replace(/^## (.*$)/gm, '<h2 class="text-xl font-bold text-white mt-6 mb-3 pb-2 border-b border-white/10">$1</h2>')
            .replace(/^### (.*$)/gm, '<h3 class="text-lg font-semibold text-white mt-4 mb-2">$1</h3>')
            .replace(/^#### (.*$)/gm, '<h4 class="text-base font-semibold text-white mt-3 mb-2">$1</h4>')
            .replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold text-white">$1</strong>')
            .replace(/\*(.*?)\*/g, '<em class="italic text-white/90">$1</em>')
            .replace(/`(.*?)`/g, '<code class="bg-dark-700 px-2 py-1 rounded text-accent-primary text-sm">$1</code>')
            .replace(/^[-*] (.*$)/gm, '<li class="text-white/80 ml-6 my-1">$1</li>')
            .replace(/^-{3,}$/gm, '<hr class="border-white/10 my-4" />')
            .replace(/\n\n/g, '</p><p class="text-white/80 my-3">')
            .replace(/\n/g, '<br/>')
    }
